...
Suite Setup
    ...
    # Install
    ${binary_url}=    Cumulocity.Create Inventory Binary    nodered    application/x-yaml    file=${CURDIR}/../testdata/docker-compose.nodered-flows-non-default-port.yaml
    ${operation}=    Cumulocity.Install Software
    ...    {"name":"nodered", "version":"1.0.0", "softwareType":"container-group", "url":"${binary_url}"}
    Operation Should Be SUCCESSFUL    ${operation}    timeout=60

    Transfer To Device    ${CURDIR}/../testdata/tedge-nodered-plugin.toml    /etc/tedge/plugins/tedge-nodered-plugin.toml
