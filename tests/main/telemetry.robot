...
node-red project should be running and processing thin-edge telemetry
    ...
    ${operation}=    Cumulocity.Execute Shell Command    text=tedge mqtt pub 'tedge/measurements' '{"temperature":10}'
    Cumulocity.Operation Should Be SUCCESSFUL    ${operation}

    # Send a big enough temperature spike to invoke the flow's logic
    ${operation}=    Cumulocity.Execute Shell Command    text=tedge mqtt pub 'tedge/measurements' '{"temperature":50}'
    Cumulocity.Operation Should Be SUCCESSFUL    ${operation}
    Cumulocity.Device Should Have Event/s    expected_text=Temperature changed by ≥10°C. new_value=50°C    after=${test_start}
